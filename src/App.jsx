...
const TYPES_BAR = ["Tous","Bar à vins","Bar lounge","Bar à cocktails","Bar à bières","Rooftop bar","Speakeasy","Bar à champagne","Cave à vins","Autre"];
const ARRONDISSEMENTS = ["Tous","75001","75002","75003","75004","75005","75006","75007","75008","75009","75010","75011","75012","75013","75014","75015","75016","75017","75018","75019","75020"];
...
const AuthModal = ({ mode, onClose, onAuth }) => {
    ...
    const validateForm = () => {
        ...
        if (tab !== "login") {
            // Nom
            const nom = sanitizeInput(form.nom.trim(), 100);
            if (!nom || nom.length < 2) {
                return "Veuillez entrer votre nom complet (min. 2 caractères).";
            }
            if (!/^[\p{L}\s'-]+$/u.test(nom)) {
            ...
            // Pro spécifique
            if (tab === "pro") {
                const etablissement = sanitizeInput(form.etablissement.trim(), 200);
                if (!etablissement || etablissement.length < 2) {
                    return "Veuillez entrer le nom de votre établissement.";
                }
                if (form.siret && !isValidSiret(form.siret)) {
                    return "Numéro SIRET invalide (14 chiffres attendus).";
                }
            }
        }
    ...
    const submit = async () => {
        ...
        const lastAttempt = sessionStorage.getItem('lastAuthAttempt');
        const now = Date.now();
        if (lastAttempt && now - parseInt(lastAttempt) < 1000) {
            setLoading(false);
            return setErr("Trop de tentatives. Veuillez patienter.");
        }
        sessionStorage.setItem('lastAuthAttempt', now.toString());
        try {
            const sanitizedEmail = sanitizeInput(form.email.trim().toLowerCase(), 254);
            const sanitizedNom = sanitizeInput(form.nom.trim(), 100);
            const sanitizedEtablissement = sanitizeInput(form.etablissement.trim(), 200);
            const sanitizedSiret = form.siret ? sanitizeInput(form.siret.replace(/\s/g, ''), 14) : null;
            if (tab === "login") {
                const { data, error } = await supabase.auth.signInWithPassword({
                    email: sanitizedEmail,
                    password: form.password, // Ne pas sanitizer le mot de passe
                });

                if (error) {
                    setLoading(false);
                    //  Message d'erreur générique pour éviter l'énumération d'utilisateurs
                    return setErr("Identifiants incorrects.");
                }
                ...
                const { data, error } = await supabase.auth.signUp({
                    ...
                    options: {
                        data: {
                            ...
                            etablissement: sanitizedEtablissement || null,
                        }
                    }
                });
                ...
                if (error) {
                    ...
                    if (error.message.includes('already registered')) {
                        return setErr("Un compte existe déjà avec cette adresse.");
                    }
            ...
            console.error('Auth error:', e); // Pour le debug, à retirer en production
...
const FicheModal = ({ lieu, user, onClose, onReserve, onAuthNeeded, onToast }) => {
    ...
    useEscapeKey(onClose);
    const submitReview = async (reviewForm, lieu, user, setAvis, setReviewSent, onAuthNeeded) => {
        if (!user) return onAuthNeeded();

        // ✅ Validation
        const texte = sanitizeInput(reviewForm.texte.trim(), 2000);
        if (!texte || texte.length < 10) {
            alert("Votre avis doit contenir au moins 10 caractères.");
            return;
        }

        const note = parseInt(reviewForm.note);
        if (isNaN(note) || note < 1 || note > 5) {
            alert("Note invalide.");
            return;
        }

        // ✅ Vérifier qu'un avis n'existe pas déjà pour cet utilisateur
        const { data: existingReview } = await supabase
            .from("avis")
            .select("id")
            .eq("etablissement_id", lieu.id)
            .eq("user_id", user.id)
            .single();

        if (existingReview) {
            alert("Vous avez déjà laissé un avis pour cet établissement.");
            return;
        }
        ...
        setAvis(a => [{
            id: data.id,
            auteur: sanitizeHTML(user.nom),
            note: note,
            date: "Maintenant",
            texte: sanitizeHTML(texte),
            verifie: true
        }, ...a]);
        setReviewSent(true);
    };
    ...
    return (
        <div style={S.overlay} onClick={onClose}>
            <div style={{ ...S.modal, maxWidth: 620, padding: 0, overflow: "hidden" }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ padding: isMobile ? "14px 14px" : "22px", maxHeight: isMobile ? "60vh" : 420, overflowY: "auto" }}>
                    {tab === "info" && (
...
const DashboardPhotos = ({ lieu, user }) => {
    ...
    const addPhotos = async (e, photos, setPhotos, user) => {
        ...
        if (photos.length + files.length > MAX_FILES) {
            alert(`Maximum ${MAX_FILES} photos autorisées.`);
            e.target.value = "";
            return;
        }
        ...
        if (invalidFiles.length > 0) {
            alert("Certains fichiers sont invalides. Formats acceptés : JPG, PNG, WebP. Taille max : 5 Mo.");
            e.target.value = "";
            return;
        }
        const newPhotos = [];
        ...
        const allPhotos = [...photos, ...newPhotos].slice(0, MAX_FILES);
        setPhotos(allPhotos);

        // Mise à jour en base
        await supabase
            .from("etablissements")
            .update({ photos: allPhotos.map(p => p.url) })
            .eq("user_id", user.id);
    ...
    return (
        <div style={{ minHeight: "100vh", background: "#F5F2EE", fontFamily: "'Lato',sans-serif" }}>
            ...
            <div style={{ maxWidth: 960, margin: "0 auto", padding: isMobile ? "16px 12px 80px" : "36px 20px" }}>
                ...
                {/* ── PHOTOS ── */}
                {proTab === "photos" && <DashboardPhotos lieu={LIEUX[0]} user={user} />}
...
export default function App() {
    ...
    if (user?.role === "pro" && proOnboarding) return <OnboardingPro user={user} onDone={async (ficheData) => {
    await supabase.from("etablissements").insert({
      ...
      price_range: ficheData.priceRange,
      ambiance: ficheData.ambiance,
      arrondissement: ficheData.arrondissement,
      places: ficheData.places,
      menu: ficheData.menu || [],
      photos: ficheData.photos || [],
    });
    user.onboarded = true;
    setProOnboarding(false);
  }} />;
